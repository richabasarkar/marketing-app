...
function Label({ children, required, tip }: { children: React.ReactNode; required?: boolean; tip?: string }) {
  return (
    <label style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 6 }}>
      ...
      {tip && <span title={tip} style={{ cursor: 'help', opacity: 0.5 }}><Info size={12} style={{ color: 'var(--text-3)' }} /></span>}
    </label>
  );
}
...
function MultiSelect({ options, value, onChange }: { options: string[]; value: string[]; onChange: (v: string[]) => void }) {
  ...
  return (
    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
      {options.map(opt => (
        <button key={opt} type="button" onClick={() => toggle(opt)} style={{ padding: '6px 14px', borderRadius: 20, fontSize: 13, fontWeight: 500, border: value.includes(opt) ? '1.5px solid var(--accent)' : '1.5px solid var(--border)', background: value.includes(opt) ? 'var(--accent-light)' : 'var(--surface)', color: value.includes(opt) ? 'var(--accent)' : 'var(--text-2)', cursor: 'pointer', transition: 'all 0.15s' }}>{opt}</button>
      ))}
    </div>
  );
}
  ...
  return (
    <div style={{ maxWidth: 860, margin: '0 auto', animation: 'fadeIn 0.4s ease' }}>
      <div style={{ marginBottom: 32 }}>
        ...
        <h1 style={{ fontFamily: 'Bricolage Grotesque', fontSize: 36, fontWeight: 800, letterSpacing: '-0.03em', marginBottom: 10 }}>
          Tell us about your <span style={{ color: 'var(--accent)' }}>company</span>
        </h1>
        <p style={{ color: 'var(--text-2)', fontSize: 16, maxWidth: 580 }}>The more detail you give us, the more personalized your strategy will be. Starred fields are required.</p>
      </div>

      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: '16px 20px', marginBottom: 24, display: 'flex', alignItems: 'center', gap: 16, boxShadow: 'var(--shadow-sm)' }}>
        <div style={{ flex: 1, height: 8, background: 'var(--surface-2)', borderRadius: 4, overflow: 'hidden' }}>
          <div style={{ height: '100%', width: `${(completedCount / requiredFields.length) * 100}%`, background: 'linear-gradient(90deg, var(--accent), #9f8fff)', borderRadius: 4, transition: 'width 0.4s ease' }} />
        </div>
      ...
      {SECTIONS.map(section => (
        <div key={section} className="card" style={{ marginBottom: 10, overflow: 'hidden' }}>
          <button type="button" onClick={() => toggleSection(section)} style={{ width: '100%', padding: '16px 20px', display: 'flex', alignItems: 'center', gap: 12, background: 'none', border: 'none', cursor: 'pointer', textAlign: 'left', borderBottom: openSections.has(section) ? '1px solid var(--border)' : 'none' }}>
            <div style={{ width: 36, height: 36, borderRadius: 9, background: 'var(--surface-2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, border: '1px solid var(--border)', flexShrink: 0 }}>{sectionConfig[section].emoji}</div>
            <div style={{ flex: 1 }}>
              <div style={{ fontFamily: 'Bricolage Grotesque', fontWeight: 700, fontSize: 15, color: 'var(--text)' }}>{sectionConfig[section].label}</div>
              <div style={{ fontSize: 12, color: 'var(--text-3)', marginTop: 1 }}>{sectionConfig[section].description}</div>
            </div>
            <div style={{ color: 'var(--text-3)' }}>{openSections.has(section) ? <ChevronUp size={16} /> : <ChevronDown size={16} />}</div>
          </button>
          ...
          {openSections.has(section) && (
            <div style={{ padding: '20px 20px 24px' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px 20px' }}>
                ...
                {section === 'product' && <>
                  <Field span={2}><Label required tip="Describe what you sell">What do you sell?</Label><textarea className="input-field" rows={3} placeholder="e.g. We make a project management app for remote teams..." value={inputs.productDescription || ''} onChange={e => set('productDescription', e.target.value)} /></Field>
                  <Field span={2}><Label required tip="What makes you different?">Your Unique Value Proposition</Label><textarea className="input-field" rows={2} placeholder="e.g. Unlike Asana, our app takes 5 minutes to set up..." value={inputs.uniqueValueProp || ''} onChange={e => set('uniqueValueProp', e.target.value)} /></Field>
                  ...
                  <Field><Label>Product Category</Label><input className="input-field" placeholder="e.g. Project management software..." value={inputs.productCategory || ''} onChange={e => set('productCategory', e.target.value)} /></Field>
                </>}
                ...
                {section === 'financial' && <>
                  ...
                  <Field><Label tip="Cost to acquire one customer">Current CAC</Label><input className="input-field" placeholder="e.g. $50, Unknown" value={inputs.currentCAC || ''} onChange={e => set('currentCAC', e.target.value)} /></Field>
                  <Field><Label tip="How much a customer is worth over their lifetime">Customer LTV</Label><input className="input-field" placeholder="e.g. $500, Unknown" value={inputs.currentLTV || ''} onChange={e => set('currentLTV', e.target.value)} /></Field>
                </>}
                ...
                {section === 'audience' && <>
                  ...
                  <Field><Label>Specific Geography</Label><input className="input-field" placeholder="e.g. New York City..." value={inputs.targetGeography || ''} onChange={e => set('targetGeography', e.target.value)} /></Field>
                  <Field span={2}><Label required>Customer Pain Points</Label><textarea className="input-field" rows={3} placeholder="What problems are your customers struggling with?" value={inputs.targetPainPoints || ''} onChange={e => set('targetPainPoints', e.target.value)} /></Field>
                  <Field span={2}><Label>Buyer Persona Description</Label><textarea className="input-field" rows={2} placeholder="e.g. 'Sarah, 32, marketing manager at a 50-person startup...'" value={inputs.buyerPersona || ''} onChange={e => set('buyerPersona', e.target.value)} /></Field>
                </>}

                {section === 'current' && <>
                  <Field span={2}><Label>Current Marketing Channels</Label>
                  ...
                  <Field><Label>Current Conversion Rate</Label>
                  ...
                  <Field span={2}><Label required>Biggest Marketing Challenge</Label><textarea className="input-field" rows={2} placeholder="e.g. 'We get traffic but no one converts...'" value={inputs.biggestMarketingChallenge || ''} onChange={e => set('biggestMarketingChallenge', e.target.value)} /></Field>
                  <Field span={2}><Label>Past Marketing Failures (optional)</Label><textarea className="input-field" rows={2} placeholder="What have you tried that didn't work?" value={inputs.pastMarketingFailures || ''} onChange={e => set('pastMarketingFailures', e.target.value)} /></Field>
                </>}

                {section === 'competition' && <>
                  <Field span={2}><Label>Main Competitors</Label><textarea className="input-field" rows={2} placeholder="e.g. Company A, Company B..." value={inputs.mainCompetitors || ''} onChange={e => set('mainCompetitors', e.target.value)} /></Field>
                  <Field span={2}><Label>Your Competitive Differentiator</Label><textarea className="input-field" rows={2} placeholder="Why would someone choose you over a competitor?" value={inputs.competitiveDifferentiator || ''} onChange={e => set('competitiveDifferentiator', e.target.value)} /></Field>
                  <Field><Label>Market Position</Label>
                ...
                {section === 'goals' && <>
                  ...
                  <Field><Label>12-Month Revenue Goal</Label><input className="input-field" placeholder="e.g. $500K, $1M..." value={inputs.revenueGoal12Months || ''} onChange={e => set('revenueGoal12Months', e.target.value)} /></Field>
                  <Field><Label>Customer Growth Goal</Label><input className="input-field" placeholder="e.g. 100 new customers..." value={inputs.customerGrowthGoal || ''} onChange={e => set('customerGrowthGoal', e.target.value)} /></Field>
                ...
                {section === 'team' && <>
                  ...
                  <Field span={2}><Label>In-house Skills</Label>
                  ...
                  <Field span={2}><Label>Anything else we should know?</Label><textarea className="input-field" rows={3} placeholder="Past wins, upcoming launches, partnerships..." value={inputs.additionalContext || ''} onChange={e => set('additionalContext', e.target.value)} /></Field>
                </>}
      ...
      <div style={{ marginTop: 28, padding: '24px 28px', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 16, display: 'flex', alignItems: 'center', justifyContent: 'space-between', boxShadow: 'var(--shadow-md)' }}>
        <div>
          <h3 style={{ fontFamily: 'Bricolage Grotesque', fontSize: 17, fontWeight: 700, marginBottom: 4 }}>Ready to generate your strategies?</h3>
          <p style={{ color: 'var(--text-3)', fontSize: 13 }}>{allRequiredDone ? '✅ All required fields complete!' : `⚠️ ${requiredFields.length - completedCount} required field(s) still needed`}</p>
        </div>
        <button className="btn-primary" onClick={onSubmit} disabled={!allRequiredDone || isLoading} style={{ padding: '13px 28px', fontSize: 15 }}>
          <Sparkles size={17} /> Generate My Strategies
        </button>
      </div>
    </div>
  );
